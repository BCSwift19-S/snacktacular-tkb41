...
class Spot: NSObject, MKAnnotation {
    ...
    func saveData(completed: @escaping (Bool) -> ()) {
        ...
        guard let postingUserID = (Auth.auth().currentUser?.uid) else {
            print("coult not save data because of postingUserID")
        ...
        if self.documentID != "" {
            ...
            ref.setData(dataToSave) { (error) in
                if let error = error {
                    print("cant updae document \(self.documentID) \(error.localizedDescription)")
                    completed(false)
                } else {
    ...
    func updateAverageRating(completed: @escaping ()->()) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error) in
            guard error == nil else {
                print("error with: \(reviewsRef.path), error: \(error!.localizedDescription)")
                return completed()
            }
            var ratingTotal = 0.0
